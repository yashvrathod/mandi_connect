...
export default function BuyerConnectionsScreen() {
    ...
    try {
      ...
      if (tab === "received") {
        ...
        const mapped = apiData
          ...
          .map((req: any) => ({
            id: req._id || req.id,
            senderName: req.senderName || req.sender?.name || "Unknown Farmer",
            senderRole: req.senderRole || "FARMER",
          ...
          <View className="flex-row items-start gap-4">
            <View className={`h-14 w-14 rounded-2xl items-center justify-center ${
              ...
              <MaterialCommunityIcons
                name={item.senderRole === "FARMER" ? "sprout" : "account-outline"}
                size={26}
            ...
            <View className="flex-1">
              <Text className="text-zinc-900 font-bold text-base mb-1">{item.senderName}</Text>
              ...
              {tab === "received" && isPending && (
                <View className="flex-row gap-2 mt-2">
                  <TouchableOpacity
                    ...
                    className="bg-brand-600 rounded-xl px-4 py-2 flex-1"
                  >
      ...
      <SafeAreaView className="flex-1 bg-gray-50">
        ...
        <View className="flex-row items-center justify-between px-5 pt-5 pb-4 bg-white border-b border-gray-100">
          <TouchableOpacity onPress={() => router.back()}>
            <MaterialCommunityIcons name="arrow-left" size={26} color="#059669" />
          </TouchableOpacity>
          <Text className="text-zinc-900 text-xl font-extrabold">Connections</Text>
          <View style={{ width: 26 }} />
        </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="flex-row items-center justify-between px-5 pt-5 pb-4 bg-white border-b border-gray-100">
        ...
        <Text className="text-zinc-900 text-xl font-extrabold">Connection Requests</Text>
        <View style={{ width: 26 }} />
      </View>
